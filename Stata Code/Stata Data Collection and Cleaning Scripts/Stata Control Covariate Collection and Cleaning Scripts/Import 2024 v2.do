...
//pulling edu for indiv
	...
	keep indiv hhid s2aq9
	rename s2aq9 head_edu
...
use "${raw_folder}/sect1_harvestw4.dta", clear
...
//HH member composition
	use "${raw_folder}/sect1_harvestw4.dta", clear
		...
		replace preteen_members = 1 if s1q4 <= 12
		
		gen child_dependent_members = 0
		replace child_dependent_members = 1 if s1q4 <= 17
		
		gen adult_member = 0 
		replace adult_member = 1 if s1q4 >= 18
		
		gen adult_male_member = 0 
		replace adult_male_member = 1 if s1q4 >= 18 & s1q2==1
	...
	preserve
		...
		save "C:\Users\\`source'\\OneDrive - The Ohio State University\RA\Data\EPAR Nigeria\Nigeria GHS W4\final_data\Nigeria_GHS_W4_household_weights.dta",replace
	restore
...
//pulling income data FROM EPAR
	...
	//dropping one missing row and total income data
		...
		keep hhid total_income w_value_assets w_total_income formal_land_rights_hh w_org_fert_rate w_inorg_fert_rate w_npk_rate w_total_harv_area_banana w_total_harv_area_beanc w_total_harv_area_cassav w_total_harv_area_cocoa w_total_harv_area_grdnt w_total_harv_area_maize w_total_harv_area_millet w_total_harv_area_rice w_total_harv_area_sorgum w_total_harv_area_soy w_total_harv_area_swtptt w_total_harv_area_yam w_kgs_harvest_banana w_kgs_harvest_beanc w_kgs_harvest_cassav w_kgs_harvest_cocoa w_kgs_harvest_grdnt w_kgs_harvest_maize w_kgs_harvest_millet w_kgs_harvest_rice w_kgs_harvest_sorgum w_kgs_harvest_soy w_kgs_harvest_swtptt w_kgs_harvest_yam w_kgs_harvest w_labor_hired w_labor_family w_labor_total w_farm_area w_land_productivity w_yield_hv_maize w_yield_hv_rice w_yield_hv_sorgum w_yield_hv_millet w_yield_hv_beanc w_yield_hv_grdnt w_yield_hv_yam w_yield_hv_swtptt w_yield_hv_cassav w_yield_hv_banana w_yield_hv_cocoa w_yield_hv_soy w_value_pro_banana w_value_pro_beanc w_value_pro_cassav w_value_pro_cocoa w_value_pro_grdnt w_value_pro_maize w_value_pro_millet w_value_pro_rice w_value_pro_sorgum w_value_pro_soy w_value_pro_swtptt w_value_pro_yam value_crop_production w_value_crop_production lvstck_holding_tlu nonfarm_income w_nonfarm_income  w_labor_hired pest_rate w_pest_rate farm_size_agland ag_hh w_farm_size_agland
	isid hhid
	tempfile 9_income
	save `9_income'
...
//flag for a household having irrigation on at least one plot
	...
	keep hhid s11b1q39
	collapse (max) s11b1q39, by(hhid)
	replace s11b1q39 = 0 if s11b1q39==1
	replace s11b1q39 = 1 if s11b1q39==2
	rename s11b1q39 irrigation_flag
...
//pulling assets
//NOTE check for erronious entries, maybe skim 99% 
	use "${raw_folder}/sect5_plantingw4.dta", clear
	
	keep hhid item_cd s5q1a s5q1 s5q2a  s5q4 
	keep if s5q1a == 1 //owned status
	//keep if s5q3 == 1 //maybe worth including 1==HH ownership 2==indiv ownership
	
	gen tot_worth = s5q1*s5q4
	//net asset calc
...
//pull land credit access
	use "${raw_folder}/sectc4c_plantingw4.dta", clear
...
//pull land price
	use "${raw_folder}/sectc4b_plantingw4.dta", clear
	
	keep lga ea /*cluster_id*/ c07q04a c07q04b c07q04c c07q04d
	rename (c07q04a c07q04b c07q04c c07q04d) (c4q4a c4q4b c4q4c c4q4d)
	isid lga ea
	tempfile 6_land_price
	save `6_land_price'

//pulling land access
	use "${raw_folder}/sectc4a_plantingw4.dta", clear
...
//pulling data on community infrastructure ie do they have local lending options
	use "${raw_folder}/sectc2_harvestw4.dta", clear
	
	keep lga ea /*cluster_id*/ is_cd c2q1 c2q3 c2q4 c2q5 c2q6b
	rename (is_cd c2q1 c2q3 c2q4 c2q5 c2q6b) (infra_code c5q1 c5q3 c5q4 c5q5 c5q6b)
...
//WAVE 4 DOES NOT HAVE QUESTION ABOUT PHONE TYPE IE SMART V DUMB s5bq8a__*
//OPTION: we could merge this onto section 1, to only keep HEADs instead of collapsing the entire family 
	use "${raw_folder}/sect4b_plantingw4.dta", clear
	
	keep hhid /*indiv*/ s4bq8 /*s5bq8a__**/ s4bq14 s4bq15__*
	rename (s4bq8 s4bq14 s4bq15__1 s4bq15__2 s4bq15__3 s4bq15__4 s4bq15__5 s4bq15__6 s4bq15__n96) (s5bq8 s5bq14 s5bq15__1 s5bq15__2 s5bq15__3 s5bq15__4 s5bq15__5 s5bq15__6 s5bq15__n96)
...
//THIS section is entirely reformulated from w4 to w5, w4 questions revolve around access to traditional banking and/or alternative systems and their accounts. nothing is mentioned of online banking --- not sure exactly how to incorporate this at the moment --- leaving out for now
	/*use "${raw_folder}/sect4a1_plantingw4.dta", clear
	exit
...
//pulling HH that didn't attempt to get loans
	use "${raw_folder}/sect4c1_plantingw4.dta" if s4cq1 == 2, clear
...
//appending and merging
	...
	use "${raw_folder}/sect4c2_plantingw4.dta", clear
	gen s4cq1 = 1
	
	keep lid zone state sector lga ea hhid lid s4cq1 s4cq4 s4cq19 s4cq20 s4cq26 s4cq2b
	rename (s4cq1 s4cq4 s4cq19 s4cq20 s4cq26 s4cq2b) (s5cq1 s5cq4 s5cq5 s5cq6 s5cq12 s5cq2b)
	...
	gen temp = 0
	replace temp = 1 if inlist(loan_reason,1,2,3,4) //this is farming imput expenses - note "BUY FARM TOOLS/IMPLEMENTS" does not fit well into wave 4 questions and is placed in other bucket
	...
	//match classifications
		...
		//Informal (community/interpersonal/trade credit)
	...
	//dropped for reason mentioned in section above
	...
	//38 observations lost in this merge --- investigate further!!!
	merge m:1 lga ea using `4_infra', generate(_m3)
		keep if _m3==3
		drop _m3
	
	//10 observations lost in this merge --- investigate further!!!
	...
	//2 observations lost in this merge --- investigate further!!!
	merge m:1 hhid using `8_net_assets', generate(_m7)
		keep if _m7==3
		drop _m7
	
	//7 observations lost in this merge --- investigate further!!! same as w5!!!
	...
	merge m:1 hhid using `11_hh_size', gen(_m10)
		keep if _m10==3
		drop _m10
	...
	save "C:\Users\\`source'\\OneDrive - The Ohio State University\RA\Data\import_data_w4_v1.dta", replace
