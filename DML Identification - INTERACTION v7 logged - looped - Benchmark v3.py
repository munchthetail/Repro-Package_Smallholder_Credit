...
def bench_txt_path(K, N_REP, y_col, drop):
    drop_lbl = safe_drop_label(drop)
    y_lbl = safe_outcome_label(y_col)
    return BENCH_DIR / f"loan_hat_oof_keys_fold{K}_Rep{N_REP}_bench_values_{y_lbl}_{drop_lbl}.txt"

def save_bench_values(K, N_REP, y_col, drop, theta_hat, sigma2_hat, nu2_hat, var_y_hat):
    path = bench_txt_path(K, N_REP, y_col, drop)
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"{y_col};{safe_drop_label(drop)};{theta_hat};{sigma2_hat};{nu2_hat};{var_y_hat}\n")
    return path

def load_bench_values(K, N_REP, y_col, drop):
    path = bench_txt_path(K, N_REP, y_col, drop)
    if not path.exists():
    ...
    parts = line.split(";")
    if len(parts) != 6:
        raise ValueError(f"Expected 6 fields in {path}, got {len(parts)}")
    y_read, drop_read, theta, sigma2, nu2, var_y = parts
    return {
        "path": str(path),
        "y_col": y_read,
        "drop": drop_read,
    ...
    try:
        ...
        for y_col in outcome_vars:
            ...
            for K in k_fold_vector:
                ...
                S_mean = data_for_dml_sub[S_col].mean()
                S_centered_vec = (data_for_dml_sub[S_col].to_numpy() - S_mean).reshape(-1, 1)
                loan_x_size_hat = loan_hat_sub * S_centered_vec

                external_predictions = {
                    "loan": {"ml_m": loan_hat_sub},
                }
                ...
                try:
                    theta_hat  = float(np.asarray(plr.coef).reshape(-1)[0])
                    sigma2_hat = float(np.mean(np.asarray(plr.sensitivity_elements["sigma2"])[0, :, 0]))
                    nu2_hat    = float(np.mean(np.asarray(plr.sensitivity_elements["nu2"])[0, :, 0]))
                    ...
                    print(f"    Could not retrieve sensitivity elements: {e}", file=log_f)
                    theta_hat, sigma2_hat, nu2_hat, var_y_hat = np.nan, np.nan, np.nan, np.nan

                # --- SAVE BENCHMARK VALUES ---
                try:
                    file_path = save_bench_values(
                        K=K, N_REP=N_REP, y_col=y_col, drop=drop,
                        theta_hat=theta_hat, sigma2_hat=sigma2_hat,
                        nu2_hat=nu2_hat, var_y_hat=var_y_hat
                    )
                    print(f"\n>>> Benchmark values saved to:\n    {file_path}", file=log_f)
                except Exception as e:
                    print(f"    Could not save benchmark values: {e}", file=log_f)
...
try:
    ...
    for K in k_fold_vector:
        ...
        for y_col in outcome_vars:
            print(f"\nOutcome: {y_col}")
            try:
                base_vals = load_bench_values(K=K, N_REP=N_REP, y_col=y_col, drop="")
                print(f"  Baseline loaded from: {base_vals['path']}")
            except Exception as e:
                print(f"  Could not load baseline for {y_col}: {e}")
                continue
            for drop in bench_set:
                try:
                    short_vals = load_bench_values(K=K, N_REP=N_REP, y_col=y_col, drop=drop)
                    ...
                    if product < 0:
                        print(f"        bound:              [not defined — c_d < 0, not a treatment confounder]")
                    elif np.isnan(rho):
                        print(f"        bound:              [rho undefined — denominator non-positive]")
                    else:
                        bias_adversarial = np.sqrt(product)
                        bias_empirical   = abs(rho) * np.sqrt(product)
        ...
        for y_col in outcome_vars:
            ...
            for drop in bench_set:
                ...
                try:
                    ...
                    if product >= 0 and not np.isnan(rho):
                    ...
                    print(f"    CSV: skipped drop={col_label} for {y_col}: {e}")

            csv_df = pd.DataFrame(csv_data)
            y_lbl = safe_outcome_label(y_col)
            csv_path = OUTPUT_DIR / f"benchmark_K{K}_Rep{N_REP}_{y_lbl}.csv"
